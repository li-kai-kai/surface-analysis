...
with st.sidebar:
    ...
    profile_reference_radius_min_mm = 0.0
    profile_effective_radius_mm = st.number_input(
        "剖面有效半径 (mm)",
        min_value=0.0,
        max_value=300.0,
        value=150.0,
...
if uploaded_file is None:
            ...
            with st.spinner("正在分析数据,请稍候..."):
                try:
                    ...
                    metrics = process_xyz(
                        ...
                        profile_reference_radius_min=profile_reference_radius_min_mm * 0.001,
                        profile_effective_radius=profile_effective_radius_mm * 0.001,
                    )
                    ...
                    st.session_state.analysis_results = {
                        ...
                        "profile_reference_radius_min_mm": profile_reference_radius_min_mm,
                        "profile_effective_radius_mm": profile_effective_radius_mm,
                    }
    ...
    if st.session_state.analysis_results is not None:
        ...
        # 3. 展示图表
        if metrics and "figures" in metrics and "profile" in metrics["figures"]:
            st.subheader("径向剖面图")
            st.plotly_chart(metrics["figures"]["profile"], width="stretch")
            st.markdown("---")
